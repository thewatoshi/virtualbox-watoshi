 ...
 *
 * This code is shared between ring-3 and ring-0.
 */
 ...
 * properties.
 *
 * @remarks typedef in VBoxGuestLibGuestProp.h.
 */
 ...
 * @param   pClient             Where to return the connection details.
 */
DECLVBGL(int) VbglGuestPropConnect(PVBGLGSTPROPCLIENT pClient)
{
#ifdef IN_RING0
    int rc = VbglR0HGCMConnect(&pClient->handle, "VBoxGuestPropSvc", &pClient->idClient);
#else
    int rc = VbglR3HGCMConnect("VBoxGuestPropSvc", &pClient->idClient);
#endif
 ...
 *
 * @returns VBox status code.
 * @param   pClient         The client session state returned by
 *                          VbglR0InfoSvcConnect().
 */
DECLVBGL(int) VbglGuestPropDisconnect(PVBGLGSTPROPCLIENT pClient)
{
    ...
    {
        VbglR0HGCMDisconnect(pClient->handle, pClient->idClient);
        pClient->idClient = 0;
        pClient->handle   = NULL;
    }
    /* else: not connected */
...
{
#ifdef IN_RING0
    return VbglR0HGCMCall(pClient->handle, pData, cbData);
#else
    RT_NOREF(pClient);
    return VbglR3HGCMCall(pData, cbData);
#endif
}
...
{
    return RT_SUCCESS(VbglGuestPropReadEx(pClient, pszPropName, NULL /*ppszValue*/, NULL /* ppszFlags */, NULL /* puTimestamp */));
}
...
DECLVBGL(int) VbglGuestPropWrite(PVBGLGSTPROPCLIENT pClient, const char *pszName, const char *pszValue, const char *pszFlags)
{
    ...
    if (pszValue != NULL)
    {
        ...
        VBGL_HGCM_HDR_INIT(&Msg.hdr, pClient->idClient, GUEST_PROP_FN_SET_PROP_VALUE, 3);
        ...
        rc = vbglGuestPropDoCall(pClient, &Msg.hdr, sizeof(Msg));
    }
    else
    {
...
DECLVBGL(int) VbglGuestPropWriteValue(PVBGLGSTPROPCLIENT pClient, const char *pszName, const char *pszValue)
{
    ...
    if (pszValue != NULL)
    {
        ...
        VBGL_HGCM_HDR_INIT(&Msg.hdr, pClient->idClient, GUEST_PROP_FN_SET_PROP_VALUE, 2);
        VbglHGCMParmPtrSetString(&Msg.name, pszName);
        VbglHGCMParmPtrSetString(&Msg.value, pszValue);
        rc = vbglGuestPropDoCall(pClient, &Msg.hdr, sizeof(Msg));
    }
...
#if !defined(IN_RING0) && !defined(VBOX_VBGLR3_XSERVER)

/**
 * Write a property value where the value is formatted in RTStrPrintfV fashion.
 *
 * @returns The same as VbglGuestPropWriteValue with the addition of
 *          VERR_NO_STR_MEMORY.
 *
 * @param   pClient         The client session state returned by
 *                          VbglR0InfoSvcConnect().
...
DECLVBGL(int) VbglGuestPropWriteValueV(PVBGLGSTPROPCLIENT pClient, const char *pszName, const char *pszValueFormat, va_list va)
{
    ...
    {
        rc = VbglGuestPropWriteValue(pClient, pszName, pszValue);
        RTStrFree(pszValue);
    }
    return rc;
}
...
DECLVBGL(int) VbglGuestPropWriteValueF(PVBGLGSTPROPCLIENT pClient, const char *pszName, const char *pszValueFormat, ...)
{
    ...
    int rc = VbglGuestPropWriteValueV(pClient, pszName, pszValueFormat, va);
    va_end(va);
    return rc;
}

#endif /* !IN_RING0 && !VBOX_VBGLR3_XSERVER */
 ...
 *                          Optional.
 */
DECLVBGL(int) VbglGuestPropRead(PVBGLGSTPROPCLIENT pClient, const char *pszName,
                                    ...
                                    uint32_t *pcbBufActual)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pClient->idClient, GUEST_PROP_FN_GET_PROP, 4);
    VbglHGCMParmPtrSetString(&Msg.name, pszName);
    VbglHGCMParmPtrSet(&Msg.buffer, pvBuf, cbBuf);
    VbglHGCMParmUInt64Set(&Msg.timestamp, 0);
    VbglHGCMParmUInt32Set(&Msg.size, 0);

    int rc = vbglGuestPropDoCall(pClient, &Msg.hdr, sizeof(Msg));
                                      ...
                                      const char *pszPropName, char **ppszValue, char **ppszFlags, uint64_t *puTimestamp)
{
    ...
    for (unsigned cTries = 0; cTries < 10; cTries++)
    {
        ...
        char    *pszFlags;
        uint64_t uTimestamp;
        rc = VbglGuestPropRead(pClient, pszPropName, pvBuf, cbBuf, &pszValue, &uTimestamp, &pszFlags, NULL);
        if (RT_FAILURE(rc))
        {
            if (rc == VERR_BUFFER_OVERFLOW)
            {
...
DECLVBGL(int) VbglGuestPropReadValueAlloc(PVBGLGSTPROPCLIENT pClient, const char *pszName, char **ppszValue)
{
    ...
    *ppszValue = NULL;
    ...
    for (unsigned i = 0; i < 10 && rc == VERR_BUFFER_OVERFLOW; ++i)
    {
        ...
        {
            pvBuf = pvTmpBuf;
            rc = VbglGuestPropRead(pClient, pszName, pvBuf, cbBuf, &pszValue, NULL, NULL, &cbBuf);
        }
        else
            rc = VERR_NO_MEMORY;
    }
...
/**
 * Free the memory used by VbglGuestPropReadValueAlloc for returning a value.
 *
                                         ...
                                         uint32_t *pcchValueActual)
{
    ...
    int rc = VbglGuestPropRead(pClient, pszName, pvBuf, cchValue, &pszValue, NULL, NULL, &cchValueActual);
 ...
 *                        small.
 */
DECLVBGL(int) VbglGuestPropEnumRaw(PVBGLGSTPROPCLIENT pClient,
 ...
 *                          VbglGuestPropEnumFree when it is no longer needed.
                                    ...
                                    char const **ppszFlags)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        rc = VbglGuestPropEnumRaw(pClient, pszzPatterns, pchBuf, cbBuf, &cbBuf);
        if (rc != VERR_BUFFER_OVERFLOW)
            break;
    ...
    {
        /*
         * Complete the handle and call VbglGuestPropEnumNext to retrieve the first entry.
         */
        ...
        if (!ppszName)
            ppszName = &pszNameTmp;
        rc = VbglGuestPropEnumNext(pHandle, ppszName, ppszValue, pu64Timestamp, ppszFlags);
        if (RT_SUCCESS(rc))
        {
            *ppHandle = pHandle;
            return rc;
 ...
 *
 * See @a VbglGuestPropEnum.
 *
 * @returns VBox status code.
 *
 * @param  pHandle       Handle obtained from @a VbglGuestPropEnum.
 * @param  ppszName      Where to store the next property name.  This will be
 *                       set to NULL if there are no more properties to
 *                       enumerate.  This pointer should not be freed. Optional.
 * @param  ppszValue     Where to store the next property value.  This will be
...
/**
 * Free an enumeration handle returned by @a VbglGuestPropEnum.
...
DECLVBGL(int) VbglGuestPropDelete(PVBGLGSTPROPCLIENT pClient, const char *pszName)
{
    ...
    VBGL_HGCM_HDR_INIT(&Msg.hdr, pClient->idClient, GUEST_PROP_FN_DEL_PROP, 1);
    VbglHGCMParmPtrSetString(&Msg.name, pszName);
    return vbglGuestPropDoCall(pClient, &Msg.hdr, sizeof(Msg));
}
 ...
 *
 * The set is specified in the same way as for VbglGuestPropEnum.
 *
 * @returns VBox status code. Stops on first failure.
 *          See also VbglGuestPropEnum.
 *
 * @param   pClient         The client session state returned by
 *                          VbglR0InfoSvcConnect().
 * @param   papszPatterns   The patterns against which the properties are
 ...
 *                          match everything.
 */
DECLVBGL(int) VbglGuestPropDelSet(PVBGLGSTPROPCLIENT pClient,
                                      ...
                                      uint32_t cPatterns)
{
    ...
    uint64_t pu64Timestamp;
    int rc = VbglGuestPropEnum(pClient,
    ...
    while (RT_SUCCESS(rc) && pszName)
    {
        ...
        rc = VbglGuestPropEnumNext(pHandle,
                                   &pszName,
                                   &pszValue,
                                   &pu64Timestamp,
                                   &pszFlags);
                                    ...
                                    uint32_t *pcbBufActual, bool *pfWasDeleted)
{
    ...
    VBGL_HGCM_HDR_INIT_TIMED(&Msg.hdr, pClient->idClient, GUEST_PROP_FN_GET_NOTIFICATION, 4, cMillies);
